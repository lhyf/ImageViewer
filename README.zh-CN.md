...
- 左侧文件夹树 —— 快速访问入口 + 磁盘,按需懒加载展开,并会自动展开定位到当前浏览的文件夹。
- **拖放**一张图片(或文件夹)到窗口,即可打开并直接定位到它所在的目录。
- 面包屑路径,配合 后退 / 前进 / 上一级 / 刷新 导航。
- 虚拟滚动缩略图网格(上千文件依旧流畅);缩略图只生成一次并缓存。
- 子文件夹排在图片前面一起列在网格中 —— 以其中第一张图片作封面并标出图片数量,双击即可进入。
...
JPG · PNG · GIF · WebP · BMP · TIFF · AVIF · **HEIC/HEIF** · 以及**相机 RAW**
(佳能 CR3/CR2、尼康 NEF、索尼 ARW、富士 RAF、Adobe DNG、奥林巴斯 ORF、
松下 RW2 等)。RAW 与 HEIC 会解码为全分辨率预览用于查看——本软件是看图器,不是 RAW 修图工具。
